...
fetchAccount[logger]: (
	...
	#let account = #match accountStripped.splitOne("@") #in {
		None #> return Err("Illegal account: {account}".format(account).into())
		Some((username, domain)) #> Account.new(username, domain, env.staticBaseUrl).try
	}

	#let subject = "acct:{}".format(account.ident)
	...
	#let accountActor = activitypub.fetchActor(
		...
		#> println("Warning: account={} is not suspended".format(account.ident))
	}
	...
	saveActorResource(
		...
		#let Some(actorId) = originalAccountIdOpt #> #match FullUrl.parse(actorId) #in {
			...
			Err(err) #> {
				println("warning: ID of actor is illegal: id={actorId}, err={err}".format())
			}
		}
	}
...
#impl self[elem]: L(elem) #> {
	#fn map[after](f: elem -> after): L(after) = {
		#match self #in {
			Nil #> Nil
